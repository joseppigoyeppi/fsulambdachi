...
const PropertiesService = {
  getScriptProperties: () => ({
    getProperty: (k) => ({ ADMIN_USERNAME: process.env.ADMIN_USERNAME ?? "tarokh", ADMIN_PASSWORD: process.env.ADMIN_PASSWORD ?? "test-password" })[k] ?? null,
  }),
};
...
context.setup();
console.log("[harness] setup() done; products:", sheets.get("products").rows.length - 1);
