...
/** Reads ?id= once on mount so dashboard links open the right order (static export: no server params). */
function useInitialId(): string | null {
...
export default function OrdersPage() {
  ...
  const matching = query ? orders.filter((o) => matchesQuery(o, query)) : orders;
                  ...
                  >
                    <td className="px-5 py-3.5">
                      <button type="button" className="block cursor-pointer text-left" aria-label={`Open order #${order.number} ${order.fullName}`}>
                        ...
                        <p className="text-xs text-foreground/60">{formatDate(order.createdAt)}</p>
                      </button>
                    </td>
    ...
    <Card className="self-start p-5 lg:sticky lg:top-24 md:p-6" aria-labelledby="order-detail-title">
      <div className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-foreground/60">{formatDate(order.createdAt)}</p>
        </div>
        <div className="flex items-center gap-2">
          <StatusBadge status={order.status} />
          <button
