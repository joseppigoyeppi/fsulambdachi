...
interface DrawerProps {
  ...
  onSignOut: () => void;
  /** Lets the account panel refresh after a successful order. */
  onOrderPlaced?: () => void;
}

export function CartDrawer(props: DrawerProps) {
  const cart = useCart();
  return <AnimatePresence>{cart.isOpen && <DrawerPanel key="panel" {...props} />}</AnimatePresence>;
}

/** Mounted only while open, so step state and focus handling reset naturally on close. */
function DrawerPanel({ settings, member, onSignOut, onOrderPlaced }: DrawerProps) {
  const cart = useCart();
      ...
      <motion.button
          ...
          >
            ...
            {step === "cart" ? (
              ...
              <CheckoutForm settings={settings} member={member} onSignOut={onSignOut} onSuccess={finish} />
            )}
      </motion.div>
    </>
  );
